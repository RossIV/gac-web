...
Greetings from {{ config('app.admin_name') }}, {{ $invitedUser->first_name }}!

You're receiving this message because {{ $teamOwner->name }} has listed you as a member of team {{ $team->name }} for {{ config('app.name') }}.

@component('mail::panel')
**Your Name:** {{ $invitedUser->name }}
**Your Knight Name:** {{ $invitedUser->alt_name }}
**Your Email:** {{ $invitedUser->email }}
**Your Phone Number:** {{ $invitedUser->phone }}
**Your Affiliation:** {{ $invitedUser->affiliationName }}
@endcomponent

If anything is not correct - please [update your profile]({{ route('profile') }}) ASAP.
If everything is correct - great! There are a few things we need you to do **before** game day:
